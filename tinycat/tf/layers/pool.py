...
def max_pool_2d_nxn_regions(inputs, output_size: int, mode: str):
    ...
    for row in range(output_size):
        for col in range(output_size):
            ...
            start_h = tf.cast(
                tf.floor(tf.multiply(tf.divide(row, n), tf.cast(h, tf.float32))),
                tf.int32,
            )
            # end_h = ceil((row + 1) / n * h)
            end_h = tf.cast(
                tf.ceil(tf.multiply(tf.divide((row + 1), n), tf.cast(h, tf.float32))),
                tf.int32,
            )
            # start_w = floor(col / n * w)
            start_w = tf.cast(
                tf.floor(tf.multiply(tf.divide(col, n), tf.cast(w, tf.float32))),
                tf.int32,
            )
            # end_w = ceil((col + 1) / n * w)
            end_w = tf.cast(
                tf.ceil(tf.multiply(tf.divide((col + 1), n), tf.cast(w, tf.float32))),
                tf.int32,
            )
