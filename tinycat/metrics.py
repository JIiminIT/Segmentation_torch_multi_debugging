...
def multi_class_dc(prediction, ground_truth):
    """Multi-class Dice coefficient function

    :param prediction: prediction input
    :param ground_truth: ground truth
    :returns: dice coefficient of each multi class label and reduce mean of it.
    """
    dcs = []
    for i in np.unique(ground_truth):
        x = np.atleast_1d(prediction == i)
        y = np.atleast_1d(ground_truth == i)
    ...
    return dcs, np.mean(dcs)


def multi_class_jc(prediction, ground_truth):
    """Multi-class Jaccard coefficient function

    :param prediction: prediction input
    :param ground_truth: ground truth
    :returns: Jaccard coefficient of each multi class label and reduce mean of it.
    """
    jcs = []
    for i in np.unique(ground_truth):
        x = np.atleast_1d(prediction == i)
...
def precision(input1, input2):
    ...
    tp = np.count_nonzero(input1 & input2)
    fp = np.count_nonzero(input1 & ~input2)
...
def recall(input1, input2):
    ...
    tp = np.count_nonzero(input1 & input2)
    fn = np.count_nonzero(~input1 & input2)
...
def assd(input1, input2, voxelspacing=None, connectivity=1):
    ...
    assd = np.mean(
        (
            asd(input1, input2, voxelspacing, connectivity),
            asd(input2, input1, voxelspacing, connectivity),
        )
    )
    return assd
...
def ravd(input1, input2):
    ...
    vol1 = np.count_nonzero(input1)
    vol2 = np.count_nonzero(input2)
...
def obj_assd(input1, input2, voxelspacing=None, connectivity=1):
    ...
    assd = np.mean(
        (
            obj_asd(input1, input2, voxelspacing, connectivity),
            obj_asd(input2, input1, voxelspacing, connectivity),
        )
    )
    return assd
...
def obj_fpr(input1, input2, connectivity=1):
    ...
    Examples
    --------
    >>> arr1 = np.asarray([[1,0,0],[1,0,1],[0,0,1]])
    >>> arr2 = np.asarray([[0,0,1],[1,0,1],[0,0,1]])
    >>> arr1
    array([[1, 0, 0],
           [1, 0, 1],
           [0, 0, 1]])
    ...
    >>> arr1 = np.asarray([1,0,1,0,1,1,1])
    >>> arr2 = np.asarray([1,1,1,0,1,0,1])
    >>> obj_fpr(arr1, arr2)
    0.3333333333333333
    >>> obj_fpr(arr2, arr1)
    0.3333333333333333

    >>> arr1 = np.asarray([1,0,1,1,1,0,1])
    >>> arr2 = np.asarray([1,1,1,0,1,1,1])
    >>> obj_fpr(arr1, arr2)
    0.0
    >>> obj_fpr(arr2, arr1)
    0.3333333333333333
    ...
    0.2
    """
    _, _, _, n_obj_input2, mapping = __distinct_binary_object_correspondences(
        input1, input2, connectivity
    )
...
def obj_tpr(input1, input2, connectivity=1):
    ...
    1.0
    """
    _, _, n_obj_input1, _, mapping = __distinct_binary_object_correspondences(
        input1, input2, connectivity
    )
